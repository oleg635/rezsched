...
if($waiver_id && $user_id)
{
	$signed_waivers = get_user_meta($user_id, 'signed_waivers');
	if($signed_waivers)
	{
		foreach($signed_waivers as $signed_waiver)
		{
			if($signed_waiver['waiver_id'] == $waiver_id)
			{
				$title = $signed_waiver['waiver_title'];
				$content = $signed_waiver['waiver_content'];
				$time_signed = $signed_waiver['time_signed'];
...
<div class="waiver-container signed-waiver-container">
...
<?php
	...
	{
?>
		<div class="etn-frontend-dashboard signed-data-container">
			<strong>Date Signed:</strong> <?=date('d/m/Y', $time_signed)?><br>
			<strong>Emergency Contact Name:</strong> <?=get_user_meta($user_id, 'contact_first_name', TRUE)?> <?=get_user_meta($user_id, 'contact_last_name', TRUE)?><br>
			<strong>Emergency Contact Number:</strong> <?=get_user_meta($user_id, 'contact_phone', TRUE)?><br>
		</div>
<?php
	}
?>
	...
	<div class="etn-frontend-dashboard" id="signed-data-container">
		...
		<div class="Facility_info">
			...
			<tr>
				...
				<th class="waivers-parent-table-th-3" scope="col">Contact name</th>
				<th class="waivers-parent-table-th-4" scope="col">Contact number</th>
			</tr>
			</thead>
			<tbody>
<?php
...
?>
			<tr>
				...
				<td class="waivers-parent-table-td-3"><?=get_user_meta($result->user_id, 'contact_first_name', TRUE)?> <?=get_user_meta($result->user_id, 'contact_last_name', TRUE)?></td>
				<td class="waivers-parent-table-td-4"><?=get_user_meta($result->user_id, 'contact_phone', TRUE)?></td>
			</tr>
<?php
	}
?>
