...
vd($timeframe, '$timeframe');

//if(0)
if($timeframe && empty($_REQUEST['no_cache']))
{
	$yesterday_page_file = $cache_dir . '/report_' . $current_user_id . '_' . $timeframe . '_' . date('Y-m-d', strtotime('-1 day')) . '.html';
	$yesterday_file_exists = file_exists($yesterday_page_file);
	//vd($file_exists, '$file_exists');
	if($yesterday_file_exists)
	{
		@unlink($yesterday_page_file);
	}
	
	$page_file = $cache_dir . '/report_' . $current_user_id . '_' . $timeframe . '_' . date('Y-m-d') . '.html';
	
	$file_exists = file_exists($page_file);
	vd($file_exists, '$file_exists');
	if($file_exists)
	{
	    $file_get_contents = file_get_contents($page_file);
...
function etn_get_events_activity_data($current_user_id, $activity_types, $event_types, &$events_by_activity, &$revenues)
{
   ...
   $args = [
	...
	if (!empty($_REQUEST['timeframe']) && in_array($_REQUEST['timeframe'], ['30', '365']))
	{
	...
	foreach($orders as $order)
	{
		...
		if($customer_id)
		{
			if(!empty($stored_customers[$customer_id]))
			    $customer = $stored_customers[$customer_id];
			else
			{
			    $customer = get_userdata($customer_id);
				if($customer)
					$stored_customers[$customer_id] = $customer;
			}
		...
		else
		{
			...
			$dokan_vendor_id = $current_user_id;
			
			//vd($dokan_vendor_id, 'Order ' . $order->ID . ' $dokan_vendor_id, $current_user_id == ' . $current_user_id);
		...
		if($products)
		{
			foreach($products as $product)
			{
				...
				if(!empty($stored_events[$product['product_id']]))
				{
					$event = $stored_events[$product['product_id']];
					//vd('IS IN $stored_events');
				}
				else
				{
					$event = get_post($product['product_id']);
					//vd('NOT IN $stored_events');
					$stored_events[$product['product_id']] = $event;
				}
				//vd($event, '$event');
				...
				if($event)
				{
					//if(!empty($_REQUEST['esd']) || !empty($_REQUEST['eed']) || !empty($_REQUEST['timeframe']))
					...
					$event_category = 0;
					if(!empty($stored_categories[$event->ID]))
						$event_category = $stored_categories[$event->ID];
					else
					{
						$categories = wp_get_post_terms($event->ID, 'etn_category');
						if(!empty($categories[0]))
							$event_category = $categories[0]->term_id;
						$stored_categories[$event->ID] = $event_category;
					}
					...
					$event_activity = 0;
					if(!empty($stored_activities[$event->ID]))
						$event_activity = $stored_activities[$event->ID];
					else
					{
						...
						if(!empty($tags[0]))
						{
							...
							$stored_activities[$event->ID] = $event_activity;
						}
					}
	...
	if(!empty($_REQUEST['timeframe']))
	{
		...
		$events_by_activity_file = $cache_dir . '/events_by_activity_' . $current_user_id . '_' . $_REQUEST['timeframe'] . '_' . date('Y-m-d') . '.json';
...
foreach($events_by_activity as $activity_name => $activity)
{
	...
	ob_start();
?>
<tr id="<?=$activity_class?>" class="activity">
	<td onclick="expand_children('<?=$activity_class?>')" class="heading_td order-events-table-td-1" colspan="6"><span id="arrow_<?=$activity_class?>" class="<?=$arrow_class?>"></span><?=$activity_name?></td>
	<td class="price-column"><?=wc_price($revenue_activity)?></td>
</tr>
<?php
	...
	//if($timeframe)
	{
		...
		if($dir_ok) {
			if($timeframe)
				$tr_file = $cache_activity_dir . '/tr-' . $timeframe . '.html';
			else
	...
	foreach($activity as $event_type_name => $events)
	{
		if($event_type_name === 'Lessons')
		{
			...
			//$event_type_class = $activity_class . '-' . strtolower(preg_replace('/[^A-Za-z]+/', '-', $event_type_name));
			$event_type_name_valid = preg_replace('/[^A-Za-z]+/', '-', $event_type_name);
			$event_type_class = $activity_class . '_' . $event_type_name_valid;
			ob_start();
?>
<tr id="<?=$event_type_class?>" class="event-data-tr event-type child-<?=$activity_class?> subchild-<?=$activity_class?>" style="display:none;">
	<td onclick="expand_children('<?=$event_type_class?>')" class="heading_td order-events-table-td-1" colspan="6"><span id="arrow_<?=$event_type_class?>" class="<?=$arrow_class?>"></span><?=$event_type_name?></td>
<?php
			$revenue_event_type = 0;
			foreach($events as $event)
				if(!empty($revenues[$event->order_id]['revenue']))
					$revenue_event_type += $revenues[$event->order_id]['revenue'];
?>
	<td class="price-column"><?=wc_price($revenue_event_type)?></td>
</tr>
<?php
			$tr_html = ob_get_clean();
			echo $tr_html;
			...
			if($timeframe)
			{
				...
				if($type_dir_ok) {
					if($timeframe)
						$tr_file = $cache_type_dir . '/tr-' . $timeframe . '.html';
					else
			...
			{
				//$lesson_type_class = $event_type_class . '-' . strtolower(preg_replace('/[^A-Za-z0-9]+/', '-', $lesson_type_name));
				$lesson_type_class = $event_type_class . '-' . preg_replace('/[^A-Za-z0-9]+/', '-', $lesson_type_name);
?>
<tr class="lesson-type child-<?=$event_type_class?> subchild-<?=$activity_class?> subchild-<?=$event_type_class?>" style="display:none;">
	<td class="order-events-table-td-1" colspan="6"><?=$lesson_type_name?></td>
<?php
				$revenue_lesson_type = 0;
				foreach($events as $event)
					if(!empty($revenues[$event->order_id]['revenue']))
						$revenue_lesson_type += $revenues[$event->order_id]['revenue'];
?>
	<td class="price-column"><?=wc_price($revenue_lesson_type)?></td>
</tr>
<tr id="child-<?=$event_type_class?>" class="event-data-tr event-data-sub-head child-<?=$event_type_class?> subchild-<?=$activity_class?> subchild-<?=$event_type_class?>" style="display:none;">
	<td class="event-data-td">Date Purchased</td>
	<td class="event-data-td">Order</td>
	<td class="event-data-td">Event Name</td>
	<td class="event-data-td">Date of Event</td>
	<td class="event-data-td">Client</td>
	<td class="event-data-td">Status</td>
	<td class="event-data-td">Revenue</td>
</tr>
<?php
				foreach($events as $event)
				{
...
?>
<tr class="event-data-tr status-paid child-<?=$event_type_class?> subchild-<?=$activity_class?> subchild-<?=$event_type_class?>" style="display:none;">
	<td class="event-data-td"><?=$event->order_datetime?></td>
	<td class="event-data-td"><a href="/order/<?=$event->order_id?>">#<?=$event->order_id?></a></td>
	<td class="event-data-td"><a class="event-link" href="/orders/<?=$event->ID?>/"><?=$event->post_title?></a></td>
	<td class="event-data-td"><?=$event->datetime?></td>
	<td class="event-data-td"><?=$event->customer_link?><?=$event->attendee_names_html?></td>
	<td class="event-data-td event-data-status"><span class="status-paid"><?=$display_status?></span></td>
	<td class="event-data-td"><?=$gross_total?></td>
</tr>
<?php
...
?>
<tr class="event-data-tr status-<?=$event->order_status?> child-<?=$event_type_class?> subchild-<?=$activity_class?> subchild-<?=$event_type_class?>" style="display:none;">
	<td class="event-data-td"><?=$refund_date->date('n/j/y')?></td>
	<td class="event-data-td"><a href="/order/<?=$event->order_id?>">#<?=$refund->ID?></a></td>
	<td class="event-data-td"><a class="event-link" href="/orders/<?=$event->ID?>/"><?=$event->post_title?></a></td>
	<td class="event-data-td"><?=$event->datetime?></td>
	<td class="event-data-td"><?=$event->customer_link?><?=$event->attendee_names_html?></td>
	<td class="event-data-td event-data-status"><span class="status-<?=$event->order_status?>"><?=$refund_status?></span></td>
	<td class="event-data-td">-<?=$refund_total?></td>
</tr>
<?php
					}
				}
...
<?php
			...
			ob_start();
?>
<tr id="child-<?=$event_type_class?>" class="event-data-tr event-data-sub-head child-<?=$event_type_class?> subchild-<?=$activity_class?>" style="display:none;">
	<td class="event-data-td">Date Purchased</td>
	<td class="event-data-td">Order</td>
	<td class="event-data-td">Event Name</td>
	<td class="event-data-td">Date of Event</td>
...
<?php
			foreach($events as $event)
			{
				...
				if(!empty($event->stripe_url))
					$display_status = '<a target="_blank" href="' . $event->stripe_url . '">' . $display_status . '</a>';
?>
<tr class="event-data-tr status-paid child-<?=$event_type_class?> subchild-<?=$activity_class?>" style="display:none;">
	<td class="event-data-td"><?=$event->order_datetime?></td>
	<td class="event-data-td"><a href="/order/<?=$event->order_id?>">#<?=$event->order_id?></a></td>
	<td class="event-data-td"><a class="event-link" href="/orders/<?=$event->ID?>/"><?=$event->post_title?></a></td>
	<td class="event-data-td"><?=$event->datetime?></td>
	<td class="event-data-td"><?=$event->customer_link?></td>
	<td class="event-data-td event-data-status"><span class="status-paid"><?=$display_status?></span></td>
	<td class="event-data-td"><?=$gross_total?></td>
</tr>
<?php
				...
				foreach($refunds as $refund)
				{
					...
					if(!empty($event->stripe_url))
						$refund_status = '<a target="_blank" href="' . $event->stripe_url . '">' . $refund_status . '</a>';
?>
<tr class="event-data-tr status-<?=$event->order_status?> child-<?=$event_type_class?> subchild-<?=$activity_class?>" style="display:none;">
	<td class="event-data-td"><?=$refund_date->date('n/j/y')?></td>
	<td class="event-data-td"><a href="/order/<?=$event->order_id?>">#<?=$refund->ID?></a></td>
	<td class="event-data-td"><a class="event-link" href="/orders/<?=$event->ID?>/"><?=$event->post_title?></a></td>
	<td class="event-data-td"><?=$event->datetime?></td>
...
if($ob_start)
{
	...
	$cache_dir = $_SERVER['DOCUMENT_ROOT'] . '/wp-content/json-cached-data';
	$page_file = $cache_dir . '/report_' . $current_user_id . '_' . $timeframe . '_' . date('Y-m-d') . '.html';
    file_put_contents($page_file, $page_html);
}
?>