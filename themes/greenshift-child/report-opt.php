...
if(0)
{
	...
	if(!file_exists($events_by_activity_file))
	{
		echo '!file_exists';
...
/*
vd($_REQUEST['psd'], '$_REQUEST[\'psd\']');
vd($_REQUEST['ped'], '$_REQUEST[\'ped\']');
vd($_REQUEST['esd'], '$_REQUEST[\'esd\']');
vd($_REQUEST['eed'], '$_REQUEST[\'eed\']');
*/
$timeframe = !empty($_REQUEST['timeframe']) ? $_REQUEST['timeframe'] : '';

$table = $wpdb->prefix . 'rez_order_summary';

$sql = 'SELECT * FROM ' . $table . ' WHERE dokan_vendor_id = "' . $current_user_id . '"';
...
if(!empty($_REQUEST['timeframe']))
{
	...
	//vd($start_param, '$start_param');
	//vd($end_param, '$end_param');
...
foreach($rows as $row)
{
	$activity = isset($activity_types[$row['event_activity']]) ? $activity_types[$row['event_activity']] : 0;
	$category = isset($event_types[$row['event_category']]) ? $event_types[$row['event_category']] : 0;
...
{
	$res = rez_populate_order_summary_incremental($per_page = 200, $order_id = 0, $current_user_id);
...
foreach($events_by_activity as $activity_name => $activity)
{
	...
	{
		//vd($event_type_name, '$event_type_name');
		//vd($events, '$events');
...
<?php
	...
	//if($timeframe)
	{
		...
		if($dir_ok) {
			...
			//vd(htmlspecialchars($tr_html), $tr_file . ' $tr_file 1');
		}
	}
	...
	foreach($activity as $event_type_name => $events)
	{
		if($event_type_name === 'Lessons')
		{
			...
			ob_start();
?>
<tr id="<?=$event_type_class?>" class="class-2 event-data-tr event-type child-<?=$activity_class?> subchild-<?=$activity_class?>" style="display:none;">
...
<?php
			...
			//if($timeframe)
			{
				...
				if($type_dir_ok) {
				    ...
				    $file_put_contents = file_put_contents($tr_file, $tr_html);
					//vd(htmlspecialchars($tr_html), $tr_file . ' $tr_file 2');
				}
			}
...
?>
<tr class="class-3 lesson-type child-<?=$event_type_class?> subchild-<?=$activity_class?> subchild-<?=$event_type_class?>" style="display:none;">
	<td class="order-events-table-td-1" colspan="6"><?=$lesson_type_name?></td>
<?php
				$revenue_lesson_type = 0;
				foreach($events as $event)
					$revenue_lesson_type += $event['revenue'];
					//if(!empty($revenues[$event['order_id']]['revenue']))
						//$revenue_lesson_type += $revenues[$event['order_id']]['revenue'];
?>
	<td class="price-column"><?=wc_price($revenue_lesson_type)?></td>
</tr>
<tr id="child-<?=$event_type_class?>" class="class-4 event-data-tr event-data-sub-head child-<?=$event_type_class?> subchild-<?=$activity_class?> subchild-<?=$event_type_class?>" style="display:none;">
...
<?php
				foreach($events as $event)
				{
					...
					//if($order_id == 12476)
						//vd($event['attendee_names_html'], '$event[\'attendee_names_html\']');
?>
<tr class="class-5 event-data-tr status-paid child-<?=$event_type_class?> subchild-<?=$activity_class?> subchild-<?=$event_type_class?>" style="display:none;">
	<td class="event-data-td"><?=$date_purchased?></td>
	<td class="event-data-td"><a href="/order/<?=$event['order_id']?>">#<?=$event['order_id']?></a></td>
	<td class="event-data-td"><a class="event-link" href="/orders/<?=$event['event_id']?>/"><?=$event['event_name']?></a></td>
	<td class="event-data-td"><?=$date_of_event?></td>
...
?>
<tr class="class-6 event-data-tr status-partial-refund child-<?=$event_type_class?> subchild-<?=$activity_class?> subchild-<?=$event_type_class?>" style="display:none;">
	<td class="event-data-td"><?=$refund_date_display?></td>
	<td class="event-data-td"><a href="/order/<?=$event['order_id']?>">#<?=$refund_id?></a></td>
	<td class="event-data-td"><a class="event-link" href="/orders/<?=$event['event_id']?>/"><?=$event['event_name']?></a></td>
	<td class="event-data-td"><?=$date_of_event?></td>
	<td class="event-data-td"><?=$event['customer_link']?><?=$event['attendee_names_html']?></td>
	<td class="event-data-td event-data-status"><span class="status-partial-refund"><?=$refund_status?></span></td>
...
<?php
			...
			//if($timeframe)
			{
				if($type_dir_ok) {
					$tr_file = $cache_type_dir . '/tr.html';
				    $file_put_contents = file_put_contents($tr_file, $tr_html);
					//vd(htmlspecialchars($tr_html), $tr_file . ' $tr_file 3');
				}
			}
		}
...
<?php
			...
			//if($timeframe)
			{
				...
				if($type_dir_ok) {
					...
					$tr_file = $cache_type_dir . '/tr.html';
				    $file_put_contents = file_put_contents($tr_file, $tr_html);
					vd(htmlspecialchars($tr_html), $tr_file . ' $tr_file 4');
				}
				*/
			}
			...
			ob_start();
?>
<tr id="child-<?=$event_type_class?>" class="class-8 event-data-tr event-data-sub-head child-<?=$event_type_class?> subchild-<?=$event_type_class?> subchild-<?=$activity_class?>" style="display:none;">
	<td class="event-data-td">Date Purchased</td>
	<td class="event-data-td">Order</td>
	<td class="event-data-td">Event Name</td>
	<td class="event-data-td">Date of Event</td>
...
<?php
			...
			//if($timeframe)
			{
				if($type_dir_ok) {
					$tr_file = $cache_type_dir . '/tr.html';
					//vd($tr_file, '$tr_file');
					//vd(htmlspecialchars($tr_html), '$tr_html');
				    $file_put_contents = file_put_contents($tr_file, $tr_html);
					//vd(htmlspecialchars($tr_html), $tr_file . ' $tr_file 5');
				}
			}
